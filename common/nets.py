...
class RSSM(common.Module):
  ...
  def init_from_vec(self, pre_state):
    x = self.get('pre_state_map1', tfkl.Dense, self._hidden, self._act)(pre_state)
    x = self.get('pre_state_map2', tfkl.Dense, self._hidden, self._act)(x)
    x = self.get('pre_state_map3', tfkl.Dense, self._hidden, self._act)(x)
    size = self._cell.state_size + self._deter
    state = self._suff_stats_layer('pre_state', x)
    state['deter'] = self.get('pre_deter', tfkl.Dense, self._deter, None)(x)
    dist = self.get_dist(state)
    state['stoch'] = dist.mode()
    return state
  ...
  def observe(self, embed, action, state=None):
    ...
    if state is None:
      ...
      state = self.init_from_vec(state)
...
class EncoderMLP(common.Module):
  ...
  def __call__(self, features):
    ...
    for index in range(self._layers):
      x = self.get(f'h{index}', tfkl.Dense, self._units, self._act)(x)
    return self.get('out', tfkl.Dense, self._shape[0], **self._out)(x)
...
class DistLayer(common.Module):
  ...
  def __call__(self, inputs):
    ...
    if self._dist == 'categorical':
      return tfd.Categorical(out)
